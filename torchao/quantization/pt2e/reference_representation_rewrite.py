...
def _reference_quantized_linear(
    ...
    acc_i32 = torch.ops.aten.linear.default(x_i8 - x_zero_point, weight_i8 - weight_zero_point, None).to(torch.int32)
    # acc_i32 = torch.ops.aten.linear.default((x_i8 - x_zero_point).to(torch.int32), (weight_i8 - weight_zero_point).to(torch.int32), None).to(torch.int32)
...
def _qdq_quantized_linear_relu(
    x_i8,
    x_scale,
    x_zero_point,
    x_quant_min,
    ...
    out_fp32 = torch.ops.aten.linear.default(x_fp32, weight_fp32, bias_fp32)
    out_fp32 = torch.ops.aten.relu_(out_fp32)
    out_i8 = torch.ops.quantized_decomposed.quantize_per_tensor(
        out_fp32, out_scale, out_zero_point, out_quant_min, out_quant_max, torch.int8
    )
    return out_i8
...
def _reference_quantized_linear_relu(
    x_i8,
    x_scale,
    x_zero_point,
    x_quant_min,
    ...
    # TODO: change to mul.Scalar when we make x_scale/weight_scale etc. Scalar values
    acc_i32 = (torch.ops.aten.mul.Tensor(acc_i32.to(torch.float32), x_scale * weight_scale / out_scale)).to(torch.int32) + out_zero_point
    out_i8 = torch.ops.aten.clamp(acc_i32, out_zero_point, out_quant_max).to(torch.int8)
    return out_i8
...
def _qdq_quantized_conv2d(
    ...
    out_fp32 = torch.ops.aten.conv2d.default(
        x_fp32,
        weight_fp32,
        bias_fp32,
        stride,
...
def _reference_quantized_conv2d(
    ...
    # acc_i32 = torch.ops.aten.conv2d.default(
    #     (x_i8 - x_zero_point).to(torch.int32),
    #     (weight_i8 - weight_zero_point).to(torch.int32),
    #     None,
    #     stride,
    #     padding,
    #     dilation,
...
def _qdq_quantized_conv2d_relu(
    x_i8,
    x_scale,
    x_zero_point,
    x_quant_min,
...
def _reference_quantized_conv2d_relu(
    x_i8,
    x_scale,
    x_zero_point,
    x_quant_min,
...
def reference_representation_rewrite(model: GraphModule) -> GraphModule:
    ...
    _REWRITE_INFO_LIST = [
        ...
        _RewriteInfo(
            _QUANTIZED_LINEAR_EXAMPLE_INPUTS,
            WrapperModule(_qdq_quantized_linear_relu),
            WrapperModule(_reference_quantized_linear_relu),
            _replace_literals_with_new_placeholders,
            _replace_literals_with_new_placeholders,
        ),
        _RewriteInfo(
        ...
        _RewriteInfo(
            _QUANTIZED_CONV2d_EXAMPLE_INPUTS,
            WrapperModule(_qdq_quantized_conv2d_relu),
            WrapperModule(_reference_quantized_conv2d_relu),
            partial(_replace_literals_with_new_placeholders, exclude_literals=[-1]),
            partial(_replace_literals_with_new_placeholders, exclude_literals=[-1]),
        ),
        _RewriteInfo(
